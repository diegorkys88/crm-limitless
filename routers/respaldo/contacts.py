...
def list_contacts(
    source: Optional[str] = Query(None),   # filter by kajabi|apollo|etc
    score:  Optional[str] = Query(None),   # filter by hot|warm|cold
    status: Optional[str] = Query(None),   # filter by pending|outreach_sent|etc
    skip:   int = Query(0),
    limit:  int = Query(50),
